...
namespace visionaray {
  ...
  struct Plane {
    inline VSNRAY_FUNC float eval(const vec3f v) const
    { return dot(v,N)-d; }
    inline VSNRAY_FUNC float eval(const vec4f v) const
    { return dot((const vec3f&)v,N)-d; }
    
    vec3f N;
    float d;
  };
  inline VSNRAY_FUNC Plane makePlane(const vec3f a,
                                     const vec3f b,
                                     const vec3f c)
  {
  ...
  inline VSNRAY_FUNC Plane makePlane(const vec4f a,
                                     const vec4f b,
                                     const vec4f c)
  {
