...
void split_primitive(aabb& L, aabb& R, float plane, int axis, dco::Instance const& prim)
{
  // empty
}

template <typename Obj>
static void getBoundsGPU(Obj obj, aabb *bounds)
{
...
template <typename Obj>
static void getPrimBoundsGPU(Obj obj, aabb *bounds)
{
  //if (blockIdx.x != 0 || threadIdx.x != 0)
  //  return;
...
{
  typedef sycl_index_bvh<basic_triangle<3,float>> TriangleBVH;
  typedef sycl_index_bvh<basic_triangle<3,float>> QuadBVH;
  typedef sycl_index_bvh<basic_sphere<float>>     SphereBVH;
  typedef sycl_index_bvh<dco::Cone>               ConeBVH;
  typedef sycl_index_bvh<basic_cylinder<float>>   CylinderBVH;
  typedef sycl_index_bvh<dco::BezierCurve>        BezierCurveBVH;
  typedef sycl_index_bvh<dco::ISOSurface>         ISOSurfaceBVH;
  typedef sycl_index_bvh<dco::Volume>             VolumeBVH;

  typedef sycl_index_bvh<dco::BLS> TLS;
  typedef sycl_index_bvh<dco::Instance> WorldTLS;
...
sycl_index_bvh<dco::BLS>::bvh_ref VisionaraySceneGPU::refBVH()
{
